...
namespace Randomizer
{
	public class CraftingRecipeAdjustments
	{
		...
		public static void HandleCraftingMenus()
		{
            ...
            //else if (genericMenu is GameMenu && Globals.Config.CraftingRecipies.TappersCantUseTapperProduce)
            //{
            //    if (!Globals.Config.CraftingRecipies.Randomize) { return; }
            //    FixTapperCost((GameMenu)genericMenu);
            //}
        }

        /// <summary>
        /// Makes the Tapper not have one of the tapper output items as an ingredient.
        /// </summary>
        /// <param name="gameMenu">The game menu that needs its cost adjusted</param>
        private static void FixTapperCost(GameMenu gameMenu)
        {
            ...
            foreach (Dictionary<ClickableTextureComponent, CraftingRecipe> page in pagesOfCraftingRecipes)
            {
                foreach (ClickableTextureComponent key in page.Keys)
                {
                    ...
                    if (recipe.name == "Tapper")
                    {
                        CraftableItem tapper = (CraftableItem)ItemList.Items[(int)ObjectIndexes.Tapper];
                        Dictionary<int, int> randomizedRecipe = tapper.LastRecipeGenerated;
                        TapperFix(page[key], randomizedRecipe);
                    }
                }
            }
        }
        ...
        /// <summary>
        /// Takes out the tapper outputs as ingredients of the tapper, changes them to wood.
        /// </summary>
        ...
        private static void TapperFix(CraftingRecipe inGameRecipe, Dictionary<int, int> randomizedRecipe)
        {
			...
			int[] Resins = {724, 725, 726};
            foreach (int id in randomizedRecipe.Keys)
            {
				if (Resins.Contains(id))
				{
					int value = randomizedRecipe[id];
					recipeList.Add(388, value * 17);
				}
				else
				{
                    int value = randomizedRecipe[id];
                    recipeList.Add(id, value);
                }
            }
        }
