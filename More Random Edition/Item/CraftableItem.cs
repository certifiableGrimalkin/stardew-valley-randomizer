...
namespace Randomizer
{
	public class CraftableItem : Item
	{
		...
		private string GetItemsRequired()
		{
			...
			switch (Category)
			{
                ...
                case CraftableCategories.Foragables:
                    ...
                    break;
                case CraftableCategories.Postgame:
                    craftingString = GetStringForPostgame();
                    break;
                case CraftableCategories.Qi:
                    craftingString = GetStringForQi();
                    break;
                default:
		...
		private string GetStringForModerateAndNeedMany()
		{
            ...
            List<Item> tempitems = GetListOfItemsForModerate();
            if (tempitems.Count == 1)
            {
                output = $"{tempitems[0].Id} {tempitems[0].GetLargeAmountRequiredForCrafting(1)}";
            }
            else
            {
		...
		private string GetStringForModerate()
		{
			...
			if (tempitems.Count == 1)
			{
				output = $"{tempitems[0].Id} {tempitems[0].GetLargeAmountRequiredForCrafting()}";
			}
			else
			{
                foreach (Item item in tempitems)
        ...
        /// <summary>
        /// One of the following, limited to one item needed
		/// - A lot of two sets of Small/No
		/// - A lot of one set of MediumTime
		...
		{
			string output = string.Empty;
			List<Item> tempitems = GetListOfItemsForDifficult();
			bool isSmallorNo = true; // Check for all the items to be at most SmallTime.
            foreach (Item item in tempitems)
            {
                isSmallorNo = isSmallorNo && (item.DifficultyToObtain == ObtainingDifficulties.NoRequirements || item.DifficultyToObtain == ObtainingDifficulties.SmallTimeRequirements);
            }
			if (isSmallorNo || tempitems.Count == 1)
			{
                foreach (Item item in tempitems)
                {
                    output += $"{item.Id} {item.GetLargeAmountRequiredForCrafting(1)} ";
                }
            }
			else
			{
        ...
        private string GetStringForDifficult()
		{
            ...
            if (isSmallorNo || tempitems.Count == 1)
            {
                foreach (Item item in tempitems)
                {
                    output += $"{item.Id} {item.GetLargeAmountRequiredForCrafting()} ";
                }
            }
            else
            {
        ...
        /// <summary>
        /// - One LongTime, one Postgame
        /// - Two Postgame
        /// - One set of Postgame, two MediumTime or less
        /// - A lot of LongTime
        /// </summary>
        /// <returns>The item string</returns>
        private string GetStringForPostgame()
        {
            List<Item> possibleItems = ItemList.Items.Values.ToList();
            Item item1, item2, item3;
            switch (Globals.RNG.Next(0, 4))
            {
                case 0:
                    item1 = ItemList.GetRandomCraftableItem(
                        new List<ObtainingDifficulties> { ObtainingDifficulties.PostgameItems },
                        this
                    );
                    item2 = ItemList.GetRandomCraftableItem(
                        new List<ObtainingDifficulties> { ObtainingDifficulties.LargeTimeRequirements },
                    ...
                    return $"{item1.GetStringForCrafting()} {item2.GetStringForCrafting()}";
                case 1:
                    item1 = ItemList.GetRandomCraftableItem(
                        new List<ObtainingDifficulties> { ObtainingDifficulties.PostgameItems },
                        this
                ...
                case 3:
                    ...
                    return $"{item1.Id} {item1.GetLargeAmountRequiredForCrafting()}";
            }
        ...
        /// - One LongTime, one Qi
        /// - One Qi, one Postgame
        /// - One set of Qi, two MediumTime or less
        /// - A lot of LongTime
        /// </summary>
        /// <returns>The item string</returns>
        private string GetStringForQi()
        {
            List<Item> possibleItems = ItemList.Items.Values.ToList();
            Item item1, item2, item3;
            switch (Globals.RNG.Next(0, 4))
            {
                case 0:
                    item1 = ItemList.GetRandomCraftableItem(
                        new List<ObtainingDifficulties> { ObtainingDifficulties.QiItems },
                        this
                    );
                    item2 = ItemList.GetRandomCraftableItem(
                        new List<ObtainingDifficulties> { ObtainingDifficulties.LargeTimeRequirements },
